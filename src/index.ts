...
import { CasdoorConfig, PluginConfig } from './config'
...
export default class AuthCasdoorPlugin implements IPluginAuth<PluginConfig> {
  ...
  public constructor(public readonly config: PluginConfig, options: PluginOptions<PluginConfig>) {
  ...
  public authenticate(user: string, password: string, cb: AuthCallback): void {
    ...
    if (storeUser) {
      this.logger.trace({ user: storeUser }, `[casdoor authenticate] user: ${user} found in cache, authenticated with groups: @{user.groups}`);
      return cb(null, storeUser.groups);
    }

    this.logger.trace(`[casdoor authenticate] user: ${user} not found in cache`);

    this.casdoor.getAuthTokenByPassword(user, password).then(({access_token, refresh_token}) => {
      ...
      return this.casdoor.getUser(user).then((response) => {
        ...
        this.logger.trace({ user, groups }, `[casdoor authenticate] user: ${user} authenticated with groups: @{user.groups}`);
  ...
  public allow_access(user: RemoteUser, pkg: VerdaccioPackageAccess, cb: AuthAccessCallback): void {
    if (user.name) {
      ...
      this.logger.debug(`[casdoor allow_access] deny user: ${user.name} not authenticated access to package: ${pkg.name}`);
  ...
  public allow_publish(user: RemoteUser, pkg: VerdaccioPackageAccess, cb: AuthAccessCallback): void {
    ...
    if (user.groups.includes('admin')) {
      this.logger.debug({name: user.name}, '[casdoor allow_publish] user: @{name} is admin, has been granted to publish');
      return cb(null, true)
    }
  ...
  public allow_unpublish(user: RemoteUser, pkg: VerdaccioPackageAccess, cb: AuthAccessCallback): void {
    ...
    if (user.groups.includes('admin')) {
      this.logger.debug({name: user.name}, '[casdoor allow_unpublish] user: @{name} is admin, has been granted to unpublish');
      return cb(null, true)
    }
    ...
    if (pkg.name?.includes('@')) {
      ...
      this.logger.trace({name: user.name}, '[casdoor allow_unpublish] user: @{name} has not been granted to unpublish');
    }
